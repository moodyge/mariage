...
export default function Home() {
  ...
  const [newGuest, setNewGuest] = useState({ name:"", status:"En attente", age:"À définir", group:"À définir", gender:"Non précisé", languages:["Français"] as string[], tags:[] as string[] });
  ...
  useEffect(() => {
    ...
    const snapshot: AppSnapshot = { guests, capacity, tableCapacities, separationRules, priorities, roomAssignments, roomPositions, iterations, activeIteration };
  ...
  }, [guests, capacity, tableCapacities, separationRules, priorities, roomAssignments, roomPositions, iterations, activeIteration, ready, isAuthenticated]);
  useEffect(() => {
  ...
  const genders = ["Femme", "Homme", "Non-binaire", "Non précisé"];
  ...
  function textEditor(g: Guest, field: "name") {
    return editKey(g.id, field) ? <input className="inline-input" autoFocus value={g[field]} onChange={e => updateGuest(g.id, field, e.target.value)} onBlur={() => setEditing(null)} onKeyDown={e => e.key === "Enter" && setEditing(null)} /> : <button className="inline-value" onClick={() => setEditing({ id: g.id, field })}>{g[field]}<span>✎</span></button>;
  }
  ...
  function openGuestEditor(guest?: Guest) {
    ...
    setNewGuest(guest ? { name:guest.name, status:guest.status, age:guest.age, group:guest.group, gender:guest.gender, languages:guest.language.split(" · ").filter(Boolean), tags:[...guest.tags] } : { name:"", status:"En attente", age:"À définir", group:"À définir", gender:"Non précisé", languages:["Français"], tags:[] });
  ...
  function saveGuest(event: React.FormEvent<HTMLFormElement>) {
    ...
    setNewGuest({ name:"", status:"En attente", age:"À définir", group:"À définir", gender:"Non précisé", languages:["Français"], tags:[] });
    setGuestModalOpen(false);
    setEditingGuestId(null);
    notify(editingGuestId !== null ? `${name} a été modifié` : `${name} a été ajouté`);
  }
  const iterationControls = <div className="iteration-switcher"><button className={activeIteration===1?"active":""} onClick={()=>switchIteration(1)}>Itération 1</button>{iterations[2]?<button className={activeIteration===2?"active":""} onClick={()=>switchIteration(2)}>Itération 2</button>:<button onClick={createSecondIteration}>＋ Créer l’itération 2</button>}</div>;

  if (!accessReady) return <main className="access-screen" aria-busy="true" />;
  if (!isAuthenticated) return <main className="access-screen"><section className="access-card"><div className="access-brand"><span className="brandmark">P</span><div><strong>Place Parfaite</strong><small>PLAN DE TABLE DU MARIAGE</small></div></div><p className="eyebrow">ESPACE PRIVÉ</p><h1>Bienvenue</h1><p>Entrez le mot de passe partagé pour consulter et modifier le plan de table.</p><form onSubmit={authenticate}><label htmlFor="access-password">Mot de passe</label><input id="access-password" type="password" autoComplete="current-password" autoFocus value={accessPassword} onChange={event=>{setAccessPassword(event.target.value);setAccessError("");}} aria-invalid={Boolean(accessError)} aria-describedby={accessError?"access-error":undefined}/>{accessError&&<span id="access-error" role="alert">{accessError}</span>}<button className="primary" type="submit">Accéder au plan</button></form></section></main>;
  ...
  return (
    <main>
      <header className="topbar">
        ...
        <nav className={mobileMenuOpen?"mobile-open":""}><div className="mobile-nav-heading"><div><small>PLACE PARFAITE</small><b>Navigation</b></div><button aria-label="Fermer le menu" onClick={()=>setMobileMenuOpen(false)}>×</button></div>{["Invités", "Plan de table", "Plan de salle", "Tables & devises", "Règles"].map((x,index) => <button key={x} className={tab === x ? "active" : ""} onClick={() => {setTab(x);setMobileMenuOpen(false)}}><span>0{index+1}</span>{x}<i>→</i></button>)}</nav>
