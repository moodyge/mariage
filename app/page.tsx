...
const ageOrder = ["Jeune", "Jeune adulte", "Adulte", "Senior", "Senior++"];
...
function ageProximity(left: string, right: string) {
  ...
  return distance === undefined ? 50 : [100, 76, 48, 22, 0][Math.min(distance, 4)];
}
type SeparationRule = { id: string; aId: number; bId: number };
type IterationSnapshot = {
  placements: Record<number, { table?: number; overrideTable?: number }>;
...
const defaultTableNames: TableDefinition[] = [
  {
    name: "Baobab Millénaire",
    motto: "Gardien du temps, il a vu mille saisons passer",
    capacity: 6,
  },
  {
    name: "Chêne Royal",
    motto: "Force tranquille, couronne dressée vers le ciel",
    capacity: 8,
  },
  {
    name: "Jardin d’Acajou",
    motto: "Bois chaud, cœur généreux, ombre précieuse",
    capacity: 8,
  },
  { name: "Forêt d’Ébène", motto: "Noir profond, silence noble et secret" },
  {
    name: "Arbre de Vie",
    motto: "Ses racines nourrissent, ses branches protègent",
  },
  {
    name: "Canopée de Lumière",
    motto: "Là où le soleil danse entre les feuilles",
  },
  ...
  { name: "Rônier Solitaire", motto: "Droit et fier, veilleur de savane" },
  {
    ...
    motto: "Les saisons changent, mais les feuilles restent",
  },
  { name: "Noyer Centenaire", motto: "Sagesse en écorce, fruits en mémoire" },
  {
    name: "Jardin des Orchidées",
    motto: "Délicatesse suspendue à chaque branche",
  },
  { name: "Clairière Enchantée", motto: "Ici, les murmures deviennent magie" },
  { name: "Kapokier Verdoyant", motto: "Géant doux, toit des nuages" },
  {
    name: "Bosquet des Murmures",
    motto: "Écoute... le vent raconte des histoires",
  },
  { name: "Refuge des Manguiers", motto: "Ombre sucrée, pause bienheureuse" },
  {
    name: "Séquoia Majestueux",
    motto: "Toucher le ciel sans quitter la terre",
  },
  {
    name: "Forêt des Étoiles",
    motto: "La nuit, ses feuilles captent la lumière",
  },
  {
    name: "Verger des Amoureux",
    motto: "Lieu fertile où l’amour se récolte chaque jour",
  },
  {
    name: "Royaume des Fleurs",
    motto: "Couleurs en fête, parfums en couronne",
  },
  { name: "Vallée des Palmiers", motto: "Danse au rythme du vent tropical" },
  {
    name: "Bois des Rêves",
    motto: "Ferme les yeux, laisse pousser l’imaginaire",
  },
  ...
  {
    name: "Jardin des Bambous",
    motto: "Souple et fort, il chante avec le vent",
  },
];
...
function displayGuestName(guest: Pick<Guest, "name" | "gender" | "age">) {
...
function normalizeGuest(guest: Guest): Guest {
  return {
    ...
    partner: guest.partner ? cleanGuestName(guest.partner) : undefined,
    couplePartner: guest.couplePartner ? cleanGuestName(guest.couplePartner) : undefined,
  };
}

function applyFixedTables(items: Guest[]) {
  return items.map((guest) => {
    if (guest.status === "Désisté" || guest.status === "Doublon")
      return {
        ...guest,
        table: undefined,
...
function getTogetherComponents(items: Guest[]) {
  ...
  for (const guest of items)
    for (const partnerName of (guest.partner || "")
      .split(",")
      .map((name) => name.trim())
      .filter(Boolean)) {
      const partner = byName.get(normalizedName(partnerName));
      if (partner) connect(guest.id, partner.id);
    }
...
function orderGuestsForDisplay(items: Guest[]) {
  const validCouples = new Set(getCoupleGroups(items).map((group) => group[0].coupleId!));
  const units = new Map<string, Guest[]>();
  for (const guest of items) {
    const key = guest.coupleId && validCouples.has(guest.coupleId) ? `couple:${guest.coupleId}` : `solo:${guest.id}`;
    units.set(key, [...(units.get(key) || []), guest]);
  }
  return Array.from(units.values())
    .map((unit) =>
      unit.sort((left, right) => {
...
function alphabeticalGuests(items: Guest[]) {
  return [...items].sort((left, right) => left.name.localeCompare(right.name, "fr", { sensitivity: "base" }));
}

function guestSurname(guest: Guest) {
  const parts = guest.name.trim().split(/\s+/).filter(Boolean);
  if (parts.length < 2) return parts[0] || guest.name;
  const particles = new Set(["da", "de", "del", "della", "des", "do", "dos", "du", "la", "le", "van", "von", "der", "den"]);
  let surnameStart = parts.length - 1;
  while (surnameStart > 0 && particles.has(parts[surnameStart - 1].toLocaleLowerCase("fr"))) surnameStart--;
  return parts.slice(surnameStart).join(" ");
}

function publicCoupleLabel(first: Guest, second: Guest) {
  if (first.gender === "Femme" && second.gender === "Homme") [first, second] = [second, first];
  if (first.name.match(/^(col\.?|colonel)\s+/i) || second.name.match(/^(col\.?|colonel)\s+/i)) {
    return `${displayGuestName(first)} et ${displayGuestName(second)}`;
  }
  const firstSurname = guestSurname(first);
  const secondSurname = guestSurname(second);
  if (normalizedName(firstSurname) !== normalizedName(secondSurname)) return `${displayGuestName(first)} et ${displayGuestName(second)}`;

  const firstGivenNames = first.name.slice(0, Math.max(0, first.name.length - firstSurname.length)).trim();
  const secondGivenNames = second.name.slice(0, Math.max(0, second.name.length - secondSurname.length)).trim();
  const availableGivenNames = [firstGivenNames, secondGivenNames].filter(Boolean).join(" et ");
  return `M. et Mme. ${availableGivenNames ? `${availableGivenNames} ` : ""}${firstSurname}`;
}

function getPlacementGroups(items: Guest[]) {
  return getTogetherComponents(
    items.map((guest) =>
      guest.pairType === "Couple"
        ? {
            ...guest,
            pairId: undefined,
            pairType: undefined,
        ...
        : { ...guest, coupleId: undefined, couplePartner: undefined },
    ),
  );
}
...
export default function Home() {
  ...
  const [editingPlacementGroup, setEditingPlacementGroup] = useState<{
    pairIds: string[];
    memberIds: number[];
  } | null>(null);
  ...
  const filtered = useMemo(() => alphabeticalGuests(guests.filter((g) => `${g.name} ${g.group}`.toLowerCase().includes(query.toLowerCase()) && (!groupFilter || g.group === groupFilter) && (!statusFilter || g.status === statusFilter) && (!ageFilter || g.age === ageFilter))), [guests, query, groupFilter, statusFilter, ageFilter]);
  const publicGuestResults = useMemo(() => {
    const needle = normalizedName(publicGuestQuery);
    const active = guests.filter((guest) => guest.table && guest.status !== "Désisté" && guest.status !== "Doublon");
    const matchingCouples = new Set(
      active
        .filter((guest) => needle && normalizedName(guest.name).includes(needle))
        .map((guest) => guest.coupleId)
        .filter(Boolean),
    );
    return orderGuestsForDisplay(active.filter((guest) => !needle || normalizedName(guest.name).includes(needle) || (guest.coupleId && matchingCouples.has(guest.coupleId))));
  }, [guests, publicGuestQuery]);
  const publicDirectoryEntries = useMemo(() => {
    const validCouples = new Set(getCoupleGroups(publicGuestResults).map((group) => group[0].coupleId!));
    const grouped = new Map<string, Guest[]>();
    for (const guest of publicGuestResults) {
      const key = guest.coupleId && validCouples.has(guest.coupleId) ? `couple:${guest.coupleId}` : `solo:${guest.id}`;
      grouped.set(key, [...(grouped.get(key) || []), guest]);
    }
    return Array.from(grouped.values()).sort((left, right) => left[0].name.localeCompare(right[0].name, "fr", { sensitivity: "base" }));
  }, [publicGuestResults]);
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event: KeyboardEvent) => {
      if (event.key === "Escape") setGuestModalOpen(false);
    };
  ...
  const mergeSuggestions = useMemo(() => {
    ...
    for (let left = 2; left <= tableNames.length; left++) {
      ...
      for (let right = left + 1; right <= tableNames.length; right++) {
        ...
        const crossConflict = separationRules.some((rule) => (leftGuests.some((guest) => guest.id === rule.aId) && rightGuests.some((guest) => guest.id === rule.bId)) || (leftGuests.some((guest) => guest.id === rule.bId) && rightGuests.some((guest) => guest.id === rule.aId)));
        if (crossConflict) continue;
        const directions = [
          {
            source: left,
            target: right,
            sourceGuests: leftGuests,
            targetGuests: rightGuests,
          },
          {
            source: right,
            target: left,
            sourceGuests: rightGuests,
            targetGuests: leftGuests,
          },
        ].filter((option) => !option.sourceGuests.some((guest) => guest.lockedTable) && option.sourceGuests.length + option.targetGuests.length <= capacityFor(option.target));
        ...
        const score = Math.round(ageScore * 0.65 + languageScore * 0.35);
        if (score < 55) continue;
        suggestions.push({
          source: bestDirection.source,
          target: bestDirection.target,
          sourceCount: bestDirection.sourceGuests.length,
          targetCount: bestDirection.targetGuests.length,
          ageScore,
          languageScore,
          score,
        });
  ...
  useEffect(() => {
    ...
    setGuests((items) =>
      items.map((guest) =>
        guest.status === "Désisté" || guest.status === "Doublon"
          ? {
              ...guest,
              table: undefined,
              lockedTable: undefined,
      ...
      applyFixedTables(
        items.map((guest) => {
          ...
          return {
            ...
            overrideTable: placement?.overrideTable,
          };
        }),
      ),
    );
  ...
  function createSecondIteration() {
    ...
    setIterations({
      ...
      2: {
        ...
        roomAssignments: { ...snapshot.roomAssignments },
        roomPositions: snapshot.roomPositions.map((position) => ({
          ...position,
        })),
      },
    });
  ...
  function switchIteration(target: 1 | 2) {
    ...
    setIterations((current) => ({
      ...current,
      [activeIteration]: captureIteration(),
    }));
    applyIteration(targetSnapshot);
    setActiveIteration(target);
  ...
  function generate() {
    ...
    for (const guest of eligible.filter((item) => item.lockedTable || item.overrideTable)) {
      const table = (guest.lockedTable ?? guest.overrideTable)!;
      tableGuests.set(table, [...(tableGuests.get(table) || []), guest]);
    }
    ...
    for (const component of allConnectedComponents) {
      ...
      for (const guest of followers) {
        assignments.set(guest.id, targetTable);
        anchoredIds.add(guest.id);
      }
    ...
    for (let index = 0; index < baseUnits.length; index++) {
      ...
      const englishAges = Array.from(
        new Set(
          baseUnits[index]
            .filter(isEnglishOnly)
            .map((guest) => guest.age)
            .filter((age) => age !== "À définir"),
        ),
      );
      ...
      if (bridgeIndex >= 0) {
        supportedUnits.push([...baseUnits[index], ...baseUnits[bridgeIndex]]);
        usedUnits.add(index);
        usedUnits.add(bridgeIndex);
      }
    }
    const supportedEnglishIds = new Set(
      supportedUnits
        .flat()
        .filter(isEnglishOnly)
        .map((guest) => guest.id),
    ...
    const compatibility = (guest: Guest, seated: Guest[]) =>
      seated.length === 0
        ? 0
        : seated.reduce((score, other) => {
    ...
    const conflicts = (unit: Guest[], tableNumber: number) =>
      unit.some((guest) =>
        separationRules.some((rule) => {
    ...
    for (const unit of units) {
      ...
      if (candidates.length === 0) {
        ...
        const reason = capacityCandidates.length === 0 ? (unit.length > largestCapacity ? `Groupe indivisible de ${unit.length} personnes : aucune table assez grande` : `Aucune table ne possède encore ${unit.length} place(s) libre(s)`) : `Toutes les tables ayant assez de place sont bloquées par une séparation`;
        unit.forEach((guest) => (nextPlacementIssues[guest.id] = reason));
        unplaced += unit.length;
        continue;
      }
  ...
  function tableCandidateFit(guest: Guest, tableNumber: number) {
    ...
    if (!seated.length)
      return {
        score: 50,
        reasons: ["Table encore vide"],
        compatible: unit.length <= getTableCapacity(tableNumber),
      };
    ...
    const agePairs = candidateAges.flatMap((candidateAge) =>
      tableAges.map((tableAge) => ({
        candidateAge,
        tableAge,
        distance: ageDistance(candidateAge, tableAge),
      })),
    );
    ...
    const reasons = [!closestAgePair ? "Âge non renseigné" : closestAgePair.distance === 0 ? `Même âge : ${closestAgePair.candidateAge}` : closestAgePair.distance === 1 ? `Tranche voisine : ${closestAgePair.candidateAge} / ${closestAgePair.tableAge}` : closestAgePair.distance === 2 ? `Écart d’âge marqué : ${closestAgePair.candidateAge} / ${closestAgePair.tableAge}` : `Écart d’âge fort : ${closestAgePair.candidateAge} / ${closestAgePair.tableAge}`, sharedLanguages.length ? `Langue commune : ${sharedLanguages.join(", ")}` : "Aucune langue commune", newAffiliation ? "Nouvelle affiliation" : "Affiliation déjà présente"];
    return {
      score: Math.round(ageScore * 0.5 + languageScore * 0.35 + affiliationScore * 0.15),
      reasons,
      compatible: !separated && hasCapacity,
    };
  }
  function placeDirectly(guestId: number, tableNumber: number) {
    ...
    if (lockedElsewhere) {
      notify(`${lockedElsewhere.name} possède un placement imposé sur une autre table`);
      return;
    }
    ...
    if (seated.length + movable.length > getTableCapacity(tableNumber)) {
      notify(`Il manque ${seated.length + movable.length - getTableCapacity(tableNumber)} place(s) sur cette table`);
      return;
    }
    ...
    if (separation) {
      notify("Placement impossible : une règle de séparation serait violée");
      return;
    }
    setGuests((items) => items.map((guest) => (movingIds.has(guest.id) ? { ...guest, table: tableNumber, overrideTable: tableNumber } : guest)));
    setEditingTable(null);
    setTableGuestQuery("");
    notify(`${unit.length > 1 ? `${unit.length} personnes déplacées` : unit[0].name} vers la table ${displayTableNumber(tableNumber)}`);
  }
  function removeDirectly(guestId: number) {
    const unit = directPlacementUnit(guestId);
    if (unit.some((guest) => guest.lockedTable)) {
      notify("Un placement imposé ne peut pas être retiré");
      return;
    }
  ...
  function togglePositionLock(guestId: number) {
    ...
    setGuests((items) => items.map((item) => (ids.has(item.id) ? { ...item, overrideTable: shouldLock ? item.table : undefined } : item)));
  ...
  function mergeTables(source: number, target: number) {
    ...
    if (!sourceGuests.length) {
      notify("Cette table est déjà vide");
      return;
    }
    if (sourceGuests.some((guest) => guest.lockedTable)) {
      notify("Fusion impossible : la table à vider contient des placements imposés");
      return;
    }
    if (sourceGuests.length + targetGuests.length > getTableCapacity(target)) {
      notify("Fusion impossible : la table cible n’a plus assez de places");
      return;
    }
    ...
    if (conflict) {
      notify("Fusion impossible : une règle de séparation serait violée");
      return;
    }
  ...
  function updateGuest(id: number, field: keyof Guest, value: string | string[]) {
    setGuests((gs) =>
      gs.map((g) => {
        if (g.id !== id) return g;
        if (field === "status" && (value === "Désisté" || value === "Doublon"))
          return {
            ...g,
            status: value,
            table: undefined,
  ...
  function editKey(id: number, field: keyof Guest) {
    return editing?.id === id && editing.field === field;
  }
  const ages = ["À définir", "Jeune", "Jeune adulte", "Adulte", "Senior", "Senior++"];
  ...
  function addRule() {
    ...
    if (!a || !b || a.id === b.id) {
      notify("Choisissez deux invités différents dans les suggestions");
      return;
    }
        ...
        return items.map((item) =>
          joinedIds.has(item.id)
            ? {
                ...item,
                pairId,
                pairType: "À placer ensemble",
                partner: joinedNames.filter((name) => name !== item.name).join(", "),
              }
            : item,
        );
      });
    ...
    setRuleA("");
    setRuleB("");
    notify("Règle ajoutée et sauvegardée");
  }
    ...
    setGuests((items) =>
      items.map((item) =>
        !ids.has(item.id)
          ? item
          : coupleOnly
            ? { ...item, coupleId: undefined, couplePartner: undefined }
            : {
                ...item,
                pairId: undefined,
                pairType: undefined,
  ...
  function savePlacementGroupEdit() {
    if (!editingPlacementGroup || editingPlacementGroup.memberIds.length < 2) {
      notify("Un groupe doit contenir au moins deux personnes");
      return;
    }
    ...
    setGuests((items) => {
      const updated = items.map((item) => {
        ...
        if (item.pairId && oldPairIds.has(item.pairId))
          return {
            ...item,
            pairId: undefined,
            pairType: undefined,
    ...
    setEditingPlacementGroup(null);
    setPlacementMemberQuery("");
    notify("Groupe modifié sans toucher aux couples");
  }
  function textEditor(g: Guest, field: "name") {
    const finish = () => {
      updateGuest(g.id, "name", cleanGuestName(g.name));
      setEditing(null);
    };
    return editKey(g.id, field) ? (
      <input className="inline-input" autoFocus value={g[field]} onChange={(e) => updateGuest(g.id, field, e.target.value)} onBlur={finish} onKeyDown={(e) => e.key === "Enter" && finish()} />
    ) : (
      <button className="inline-value" onClick={() => setEditing({ id: g.id, field })}>
        {g[field]}
        <span>✎</span>
      </button>
    );
  ...
  function selectEditor(g: Guest, field: "age" | "group" | "gender", options: string[]) {
    return editKey(g.id, field) ? (
      <select
        ...
        value={g[field]}
        onChange={(e) => {
          updateGuest(g.id, field, e.target.value);
          setEditing(null);
        }}
      ...
      <button className={`inline-value ${field === "group" ? "pill" : ""}`} onClick={() => setEditing({ id: g.id, field })}>
        {g[field]}
        <span>⌄</span>
      </button>
    );
  ...
  function tagsEditor(g: Guest) {
    ...
    function toggle(tag: string) {
      updateGuest(g.id, "tags", g.tags.includes(tag) ? g.tags.filter((item) => item !== tag) : [...g.tags, tag]);
    }
    return editKey(g.id, "tags") ? (
      <div className="characteristic-editor">
        {options.map((tag) => (
          <label key={tag}>
            <input type="checkbox" checked={g.tags.includes(tag)} onChange={() => toggle(tag)} />
            {tag}
            {tag === "Témoin" && <b>Essentiel</b>}
          </label>
        ))}
        <button onClick={() => setEditing(null)}>Terminé</button>
      </div>
    ) : (
      <button className="tag-list" onClick={() => setEditing({ id: g.id, field: "tags" })}>
        {g.tags.length ? g.tags.map((t) => <mark key={t}>{t}</mark>) : <i>＋ Ajouter</i>}
        <span>⌄</span>
      </button>
    );
  }
  function languageEditor(g: Guest) {
    ...
    return editKey(g.id, "language") ? (
      <div className="language-editor">
        {options.map((language) => (
          <label key={language}>
            <input type="checkbox" checked={selectedLanguages.includes(language)} onChange={() => toggle(language)} />
            {language}
          </label>
        ))}
        <button onClick={() => setEditing(null)}>Terminé</button>
      ...
      <button className="inline-value language-value" onClick={() => setEditing({ id: g.id, field: "language" })}>
        {g.language}
        <span>⌄</span>
      </button>
    );
  ...
  function relationEditor(g: Guest) {
    function choose(partnerId: string) {
      if (partnerId === "__remove__") {
        ...
        setEditing(null);
        notify(`Le couple de ${g.name} a été retiré sans modifier ses groupes`);
        return;
      }
      if (!partnerId) return;
      const targetId = Number(partnerId);
      const target = guests.find((item) => item.id === targetId);
      if (!target) return;
    ...
    return editKey(g.id, "pairId") ? (
      <div className="relation-editor">
        <input
          className="inline-input relation-search"
          autoFocus
          list={`relation-options-${g.id}`}
          value={relationQuery}
          placeholder="Taper le conjoint…"
          onChange={(e) => selectTypedName(e.target.value)}
          onKeyDown={(e) => {
            if (e.key === "Escape") setEditing(null);
          }}
          onBlur={() => window.setTimeout(() => setEditing(null), 150)}
        />
        <datalist id={`relation-options-${g.id}`}>
          {candidates.map((item) => (
            <option key={item.id} value={item.name} />
          ))}
        </datalist>
        {g.coupleId && (
          <button onMouseDown={(e) => e.preventDefault()} onClick={() => choose("__remove__")}>
            Retirer le couple
          </button>
        )}
      </div>
      ...
      <button
        ...
        onClick={() => {
          setRelationQuery("");
          setEditing({ id: g.id, field: "pairId" });
        }}
      >
        {g.coupleId ? (
          <>
            <b>Couple</b>
            <small>{g.couplePartner}</small>
          </>
        ) : (
          <>＋ Noter le couple</>
        )}
  ...
  function permanentlyDelete(g: Guest) {
    ...
    setGuests((items) =>
      items
        ...
        .map((item) =>
          item.pairId && item.pairId === g.pairId
            ? {
                ...item,
                pairId: undefined,
                pairType: undefined,
    ...
    setSelected((items) => items.filter((id) => id !== g.id));
    setRowMenu(null);
    notify(`${g.name} a été supprimé définitivement`);
  }
  function statusEditor(g: Guest) {
    return editKey(g.id, "status") ? (
      <select
        className="status-select"
        autoFocus
        value={g.status}
        onChange={(e) => {
          updateGuest(g.id, "status", e.target.value);
          setEditing(null);
        }}
        onBlur={() => setEditing(null)}
      >
      ...
      <button className={`status-chip status-${g.status.toLowerCase().replace("é", "e").replace(" ", "-")}`} onClick={() => setEditing({ id: g.id, field: "status" })}>
        {g.status}
      </button>
    );
  }
  ...
  function exportRoomPdf() {
    document.body.dataset.printRoom = "true";
    const cleanup = () => {
      delete document.body.dataset.printRoom;
    };
    ...
    setNewGuest(
      guest
        ? {
            ...
            group: guest.group,
            gender: guest.gender,
            languages: guest.language.split(" · ").filter(Boolean),
            tags: [...guest.tags],
          }
        : {
            name: "",
        ...
        items.map((guest) =>
          guest.id === editingGuestId
            ? {
                ...
                language: newGuest.languages.join(" · ") || "Français",
                tags: newGuest.tags,
                ...(newGuest.status === "Désisté" || newGuest.status === "Doublon"
                  ? {
                      table: undefined,
                      lockedTable: undefined,
                      overrideTable: undefined,
  ...
  const iterationControls = (
    <>
      <button className="outline public-export" onClick={exportTablesPdf}>
        ↓ Tirage des tables PDF
      </button>
      <div className="iteration-switcher">
        <button className={activeIteration === 1 ? "active" : ""} onClick={() => switchIteration(1)}>
          Itération 1
        </button>
        {iterations[2] ? (
          <button className={activeIteration === 2 ? "active" : ""} onClick={() => switchIteration(2)}>
            Itération 2
          </button>
        ) : (
          <button onClick={createSecondIteration}>＋ Créer l’itération 2</button>
        )}
      </div>
    </>
  );
    ...
    <main className={!isAuthenticated ? "public-readonly" : undefined}>
      <header className="topbar">
        <div className="brand">
          ...
          <div>
            <strong>Place&nbsp;Parfaite</strong>
            <small>LE PLAN DE TABLE, EN HARMONIE</small>
          </div>
        </div>
        <button className="mobile-menu-trigger" aria-label="Ouvrir le menu" aria-expanded={mobileMenuOpen} onClick={() => setMobileMenuOpen(true)}>
          <span></span>
          <span></span>
          <span></span>
        </button>
        <nav className={mobileMenuOpen ? "mobile-open" : ""}>
          ...
          {(isAuthenticated ? ["Invités", "Liste alphabétique", "Plan de table", "Plan de salle", "Tables & devises", "Règles"] : ["Liste alphabétique", "Plan de table", "Plan de salle"]).map((x, index) => (
            <button
              key={x}
      ...
      {isAuthenticated && tab === "Invités" && (
        <section className="page">
          <div className="heading">
            <div>
              <p className="eyebrow">VOTRE MARIAGE · 14 JUIN 2027</p>
              <h1>La liste des invités</h1>
              <p>Organisez les affinités aujourd'hui, laissez l'algorithme trouver l'équilibre demain.</p>
            </div>
            <div className="actions">
              <button className="primary" onClick={() => openGuestEditor()}>
                ＋ Ajouter un invité
              </button>
            </div>
          </div>
          <div className="stats">
            <article>
              <span>INVITÉS ACTIFS</span>
              <b>{guests.filter((g) => g.status !== "Désisté" && g.status !== "Doublon").length}</b>
              <small>{guests.length} fiches au total</small>
            </article>
            <article>
              <span>GROUPES</span>
              <b>{new Set(guests.map((g) => g.group)).size}</b>
              <small>affiliations identifiées</small>
            </article>
            <article>
              <span>DÉSISTEMENTS</span>
              <b>{guests.filter((g) => g.status === "Désisté").length}</b>
              <small>exclus de la génération</small>
            </article>
            <article className="capacity">
              <span>CAPACITÉ PAR TABLE</span>
              <div>
                <button onClick={() => setCapacity(Math.max(2, capacity - 1))}>−</button>
                <b>{capacity}</b>
                <button onClick={() => setCapacity(capacity + 1)}>＋</button>
              </div>
              <small>Le plan sera recalculé à la demande</small>
            </article>
          </div>
          <div className="toolbar">
            <div className="search">
              ⌕ <input aria-label="Rechercher" placeholder="Rechercher un invité, une famille…" value={query} onChange={(e) => setQuery(e.target.value)} />
            </div>
            <div className="filters">
              <span>☷ Filtres {activeFilterCount > 0 && <b>{activeFilterCount}</b>}</span>
              <select aria-label="Filtrer par affiliation" value={groupFilter} onChange={(e) => setGroupFilter(e.target.value)}>
              ...
              <select aria-label="Filtrer par tranche d’âge" value={ageFilter} onChange={(e) => setAgeFilter(e.target.value)}>
            ...
            <i></i>
            <span>
              {filtered.length} affichés · {selected.length} sélectionnés
            </span>
            {selected.length > 0 && (
              <select
            ...
            {selected.length > 0 && (
              <select
                ...
                onChange={(e) => {
                  ...
                  notify(`${selected.length} statuts modifiés`);
                  e.target.value = "";
                }}
              >
                <option value="" disabled>
            ...
            <button
              onClick={() => {
                ...
                setSelected([]);
                notify("Invités marqués comme désistés");
              }}
            >
              Marquer désisté
            </button>
          ...
          <div className="tablewrap">
            <table>
              <thead>
                <tr>
                  <th>
                    <input type="checkbox" checked={selected.length === guests.length} onChange={(e) => setSelected(e.target.checked ? guests.map((g) => g.id) : [])} />
                  </th>
                  <th>INVITÉ / STATUT</th>
                  <th>TRANCHE D'ÂGE</th>
                  <th>AFFILIATION</th>
              ...
              <tbody>
                {filtered.map((g) => (
                  <tr className={g.status === "Désisté" || g.status === "Doublon" ? "inactive-row" : ""} key={g.id}>
                    <td>
                      <input type="checkbox" checked={selected.includes(g.id)} onChange={() => setSelected((s) => (s.includes(g.id) ? s.filter((id) => id !== g.id) : [...s, g.id]))} />
                    </td>
                    <td>
                      <div className="person">
                        ...
                        <div>
                          {textEditor(g, "name")}
                          {statusEditor(g)}
                          <small className="mobile-guest-summary">
                            {g.age} · {g.group} · {g.language}
                          </small>
                        </div>
                        <button className="mobile-edit-guest" onClick={() => openGuestEditor(g)}>
                          Modifier
                        </button>
                      </div>
                    </td>
                    <td>{selectEditor(g, "age", ages)}</td>
                    <td>{selectEditor(g, "group", groups)}</td>
                    <td>{selectEditor(g, "gender", genders)}</td>
                    <td>{languageEditor(g)}</td>
                    <td>{relationEditor(g)}</td>
                    <td>{tagsEditor(g)}</td>
                    <td className="row-actions">
                      <button aria-label={`Actions pour ${g.name}`} onClick={() => setRowMenu(rowMenu === g.id ? null : g.id)}>
                        •••
                      </button>
                      {rowMenu === g.id && (
                        <div className="row-menu">
                          <button
                            onClick={() => {
                              updateGuest(g.id, "status", "Désisté");
                              setRowMenu(null);
                            }}
                          >
                            Marquer désisté
                          </button>
                          <button className="danger" onClick={() => permanentlyDelete(g)}>
          ...
          <div className="bottom">
            <p>
              <b>{guests.length} invités</b> · dernière modification à l'instant
            </p>
          </div>
        </section>
      )}

      {tab === "Liste alphabétique" && (
        <section className="page public-directory-page">
          <div className="public-directory-bar">
            <div>
              <p className="eyebrow">LISTE ALPHABÉTIQUE</p>
              <h1>Trouvez votre table</h1>
            </div>
            <div className="public-directory-search">
              <span aria-hidden="true">⌕</span>
              <input autoFocus type="search" inputMode="search" autoComplete="off" aria-label="Rechercher votre nom" placeholder="Rechercher un prénom ou un nom…" value={publicGuestQuery} onChange={(event) => setPublicGuestQuery(event.target.value)} />
              {publicGuestQuery && (
                <button aria-label="Effacer la recherche" onClick={() => setPublicGuestQuery("")}>
                  ×
                </button>
              )}
            </div>
          </div>
          {publicDirectoryEntries.length ? (
            <div className="public-directory-list">
              {publicDirectoryEntries.map((entry) => {
                const guest = entry[0];
                const internalTable = guest.table!;
                const table = tableNames[internalTable - 1];
                const isCouple = entry.length > 1;
                return (
                  <article key={entry.map((item) => item.id).join("-")}>
                    <span className="public-directory-initials">
                      {isCouple ? "∞" : guest.name.split(" ").filter(Boolean).slice(0, 2).map((part) => part[0]).join("")}
                    </span>
                    <div>
                      <h2>{isCouple ? publicCoupleLabel(entry[0], entry[1]) : displayGuestName(guest)}</h2>
                      <p>{table?.name}</p>
                    </div>
                    <strong>
                      <small>TABLE</small>
                      {displayTableNumber(internalTable)}
                    </strong>
                  </article>
                );
              })}
            ...
            <div className="public-directory-empty">
              <b>Aucun invité trouvé</b>
              <p>Vérifiez l’orthographe ou essayez seulement quelques lettres.</p>
              <button onClick={() => setPublicGuestQuery("")}>Voir toute la liste</button>
            </div>
          )}
        </section>
      )}
      ...
      {tab === "Plan de table" && (
        <section className="page">
          <div className="heading">
            <div>
              <p className="eyebrow">PROPOSITION {generated ? "NOUVELLEMENT CALCULÉE" : "DE TRAVAIL"}</p>
              <h1>Votre plan de table</h1>
              <p>{isAuthenticated ? "Une composition équilibrée, modifiable par override." : "Consultez la répartition des invités par table."}</p>
            </div>
            <div className="plan-heading-actions">
              <button className="outline public-export" onClick={exportAlphabeticalSeating}>
          ...
          <div className="placement-legend">
            <span className="legend-imposed">● Placement imposé</span>
            <span className="legend-locked">● Manuel verrouillé</span>
            <span className="legend-auto">● Auto-fill</span>
          </div>
          <details className="table-merger">
            <summary>
              <div>
                <small>OUTIL DE RAPPROCHEMENT</small>
                <b>Rapprocheur de tables</b>
              </div>
              <strong>{mergeSuggestions.length} proposition(s)</strong>
            </summary>
            <p>Tables peu remplies, sans exclusion, classées selon l’âge et les langues. Une fusion remplace les verrouillages manuels.</p>
            {mergeSuggestions.length ? (
              <div className="merge-suggestions">
                {mergeSuggestions.map((suggestion) => (
                  <details className="merge-proposal" key={`${suggestion.source}-${suggestion.target}`}>
                    <summary>
                      <div>
                        <b>
                          Table {displayTableNumber(suggestion.source)} → Table {displayTableNumber(suggestion.target)}
                        </b>
                        <small>
                          {tableNames[suggestion.source - 1].name} rejoint {tableNames[suggestion.target - 1].name}
                        </small>
                      </div>
                      <span>
                        {suggestion.sourceCount} + {suggestion.targetCount}/{getTableCapacity(suggestion.target)}
                      </span>
                      <div className="merge-scores">
                        <em>Âge {suggestion.ageScore}%</em>
                        <em>Langues {suggestion.languageScore}%</em>
                        <strong>{suggestion.score}% compatible</strong>
                      </div>
                      <i>Voir les invités</i>
                    </summary>
                    <div className="merge-people">
                      <section>
                        <h4>À déplacer · Table {displayTableNumber(suggestion.source)}</h4>
                        {guests
                          .filter((g) => g.table === suggestion.source && g.status !== "Désisté" && g.status !== "Doublon")
                          .map((g) => (
                            <div key={g.id}>
                              <b>{g.name}</b>
                              <small>
                                {g.age} · {g.language} · {g.group}
                              </small>
                            </div>
                          ))}
                      </section>
                      <section>
                        <h4>Déjà présents · Table {displayTableNumber(suggestion.target)}</h4>
                        {guests
                          .filter((g) => g.table === suggestion.target && g.status !== "Désisté" && g.status !== "Doublon")
                          .map((g) => (
                            <div key={g.id}>
                              <b>{g.name}</b>
                              <small>
                    ...
                    <div className="merge-confirm">
                      <small>La fusion remplacera les verrouillages manuels de ces deux tables.</small>
                      <button onClick={() => mergeTables(suggestion.source, suggestion.target)}>Fusionner · écraser les verrous</button>
                    </div>
                  </details>
                ))}
              </div>
            ) : (
              <div className="merge-empty">Aucune fusion compatible pour le moment. Les capacités, séparations ou placements imposés empêchent le rapprochement.</div>
            )}
          </details>
          <details className={`unplaced-panel ${unplacedGuests.length ? "has-unplaced" : "complete"}`}>
            <summary>
              <div>
                <small>SUIVI DU PLACEMENT</small>
                <b>Reste à placer</b>
              </div>
              <strong>{unplacedGuests.length}</strong>
            </summary>
            {unplacedGuests.length ? (
              <>
                <div className="unplaced-actions">
                  <b>Diagnostic des blocages</b>
                  <button onClick={() => setTab("Règles")}>Gérer les règles →</button>
                </div>
                <div className="unplaced-list">
                  {unplacedGuests.map((guest) => (
                    <span key={guest.id}>
                      {guest.name}
                      <small>
                        {guest.age} · {guest.group}
                      </small>
                      <em>{placementIssues[guest.id] || "Recalculez le plan pour identifier le blocage"}</em>
                    </span>
                  ))}
                </div>
              </>
            ) : (
              <p>Tout le monde a une table.</p>
            )}
          </details>
          <div className="planboard">
            {tableNames.map((t, i) => {
              const seated = orderGuestsForDisplay(guests.filter((g) => g.table === i + 1 && g.status !== "Désisté" && g.status !== "Doublon"));
              const tableCapacity = getTableCapacity(i + 1);
              return (
                <article className="tablecard" key={t.name}>
                  <div className="tabletitle">
                    <div>
                      <small>{i === 0 ? "TABLE 0 · TABLE DES MARIÉS" : `TABLE ${i}`}</small>
                      <h2>{t.name}</h2>
                      <p>« {t.motto} »</p>
                    </div>
                    <b>
                      {seated.length}/{tableCapacity}
                    </b>
                  </div>
                  <div className="seats">
                    {seated.map((g, seatIndex) => {
                      const isSecondHalfOfCouple = Boolean(
                        g.coupleId && seatIndex > 0 && seated[seatIndex - 1].coupleId === g.coupleId,
                      );
                      return (
                      <button
                        key={g.id}
                        disabled={!isAuthenticated}
                        className={`${g.lockedTable ? "seat-imposed" : g.overrideTable ? "seat-locked" : "seat-auto"} ${isSecondHalfOfCouple ? "couple-linked" : ""}`}
                        onClick={() => {
                          setEditingTable(i + 1);
                          setTableGuestQuery("");
                        }}
                      >
                        ...
                        {displayGuestName(g)}
                        <small>{g.age} · {g.lockedTable ? "Placement imposé" : g.overrideTable ? "🔒 Manuel" : `✦ Auto-fill · ${g.group}`}</small>
                      </button>
                      );
                    })}
                    {Array.from({
                      length: Math.max(0, Math.min(tableCapacity - seated.length, 2)),
                    }).map((_, j) => (
                      <button
                        className="empty"
                        key={j}
                  ...
                  {editingTable === i + 1 && (
                    <div className="direct-table-editor">
                      <div className="direct-editor-title">
                        <div>
                          <b>Modifier la table {i}</b>
                          <small>Ajoutez, déplacez ou retirez des invités. Couples et jumeaux suivent automatiquement.</small>
                        </div>
                        <button onClick={() => setEditingTable(null)}>×</button>
                      </div>
                      <div className="direct-current">
                        {seated.map((g) => (
                          <span key={g.id}>
                            {g.name}
                            {g.lockedTable ? (
                              <small>Placement imposé</small>
                            ) : (
                              <div className="direct-seat-actions">
                                <button onClick={() => togglePositionLock(g.id)}>{g.overrideTable ? "Déverrouiller" : "Verrouiller"}</button>
                                <button onClick={() => removeDirectly(g.id)}>Retirer</button>
                              </div>
                            )}
                          </span>
                        ))}
                      </div>
                      <input autoFocus value={tableGuestQuery} onChange={(e) => setTableGuestQuery(e.target.value)} placeholder="Rechercher un invité à ajouter ou déplacer…" />
                      <div className="direct-suggestion-label">
                        <b>Suggestions pour cette table</b>
                        <small>Non placés en priorité · âge, langues et affiliation</small>
                      </div>
                      <div className="direct-candidates">
                        {guests
                          .filter((g) => g.status !== "Désisté" && g.status !== "Doublon" && !g.lockedTable && g.table !== i + 1 && g.name.toLowerCase().includes(tableGuestQuery.toLowerCase()))
                          .map((g) => ({
                            guest: g,
                            fit: tableCandidateFit(g, i + 1),
                          }))
                          .sort((a, b) => Number(Boolean(a.guest.table)) - Number(Boolean(b.guest.table)) || Number(b.fit.compatible) - Number(a.fit.compatible) || b.fit.score - a.fit.score || a.guest.name.localeCompare(b.guest.name))
                          .slice(0, 30)
                          .map(({ guest: g, fit }) => (
                            <button key={g.id} disabled={!isAuthenticated} className={!fit.compatible ? "candidate-incompatible" : ""} onClick={() => placeDirectly(g.id, i + 1)}>
                              <div>
                                <span>{g.name}</span>
                                <strong>{fit.compatible ? `${fit.score}%` : "Bloqué"}</strong>
                              </div>
                              <small>
                                {g.table ? `Table ${displayTableNumber(g.table)} · déplacer` : `Reste à placer`} · {fit.reasons.join(" · ")}
                              </small>
                            </button>
                          ))}
                      </div>
                  ...
                  <footer>
                    <span>{seated.some((g) => g.lockedTable) ? <b>Placements imposés</b> : <>Table modifiable</>}</span>
                    <button
                      onClick={() => {
                        setEditingTable(i + 1);
                        setTableGuestQuery("");
      ...
      {tab === "Plan de salle" && (
        <section className="page room-page">
          <div className="heading">
            <div>
              <p className="eyebrow">DISPOSITION DE LA SALLE · D’APRÈS LA PHOTO</p>
              <h1>Plan visuel de la salle</h1>
              <p>{isAuthenticated ? "Choisissez une table sur chaque emplacement. Une réattribution échange automatiquement les deux tables." : "Consultez la disposition des tables et leurs invités dans la salle."}</p>
            </div>
            <div className="room-heading-actions">
              {iterationControls}
              <button className="outline" onClick={exportRoomPdf}>
                ↓ Imprimer / enregistrer en PDF
              </button>
              <button
                className="outline"
                onClick={() => {
                  setRoomAssignments(Object.fromEntries(defaultRoomPositions.map((position) => [position.slot, position.slot + 1])));
                  setRoomPositions(defaultRoomPositions);
                  notify("Disposition de la photo restaurée");
                }}
              >
          ...
          <div
            ...
            onDrop={(e) => {
              e.preventDefault();
              if (draggedRoomSlot !== null) moveRoomPosition(draggedRoomSlot, e.clientX, e.clientY, e.currentTarget.getBoundingClientRect());
              setDraggedRoomSlot(null);
            }}
          >
            <div className="room-feature honor-backdrop">Backdrop · coin photo</div>
            <div className="room-feature dance-floor">
              Piste
              <br />
              de danse
            </div>
            <div className="room-feature left-screen" aria-hidden="true"></div>
            <div className="room-feature right-screen" aria-hidden="true"></div>
            <div className="room-feature bar">Bar</div>
            <div className="room-feature dj">DJ</div>
            <div className="room-feature entrance">Entrée</div>
            {roomPositions.map((position) => {
              const internalTable = roomAssignments[position.slot] ?? position.slot + 1;
              const seated = orderGuestsForDisplay(guests.filter((g) => g.table === internalTable && g.status !== "Désisté" && g.status !== "Doublon"));
              return (
                <article key={position.slot} tabIndex={0} className={`room-table ${position.slot === 0 ? "honor-table" : ""} ${position.y < 20 ? "tooltip-below" : ""} ${draggedRoomSlot === position.slot ? "is-dragging" : ""}`} style={{ left: `${position.x}%`, top: `${position.y}%` }}>
                  <b className="room-screen-label">{position.slot === 0 ? "Table 0 · Mariés" : `Emplacement ${position.slot}`}</b>
                  <b className="room-print-label">Table {displayTableNumber(internalTable)}</b>
                  <strong>{tableNames[internalTable - 1]?.name}</strong>
                  <small>
                    {seated.length}/{getTableCapacity(internalTable)} personnes
                  </small>
                  <button
                    type="button"
                    ...
                    draggable
                    onDragStart={(e) => {
                      setDraggedRoomSlot(position.slot);
                      e.dataTransfer.effectAllowed = "move";
                    }}
                    onDragEnd={() => setDraggedRoomSlot(null)}
                  >
                    ⠿ Déplacer
                  </button>
                  {position.slot > 0 && (
                    <select aria-label={`Table installée à l’emplacement ${position.slot}`} value={internalTable} onChange={(e) => assignTableToRoomSlot(position.slot, Number(e.target.value))}>
                      {tableNames.slice(1).map((table, index) => (
                        <option key={table.name} value={index + 2}>
                          Table {index + 1} · {table.name}
                        </option>
                      ))}
                    </select>
                  )}
                  <div className="room-tooltip" role="tooltip">
                    <b>Invités de la table</b>
                    {seated.length ? (
                      <ul>
                        {seated.map((guest) => (
                          <li key={guest.id}>
          ...
          <p className="room-note">Disposition approximative reproduite depuis le croquis. La table 0 reste fixée en haut ; les tables 1 à 26 peuvent être échangées librement.</p>
        </section>
      )}

      {isAuthenticated && tab === "Tables & devises" && (
        <section className="page narrow">
          <div className="heading">
            <div>
              <p className="eyebrow">L'IDENTITÉ DE VOTRE RÉCEPTION</p>
              <h1>Tables & devises</h1>
              <p>Chaque modification est sauvegardée automatiquement et utilisée dans les plans et PDF.</p>
            </div>
          </div>
          <div className="table-swap-tool">
            <div>
              <small>ÉCHANGER DEUX IDENTITÉS</small>
              <b>Permuter les noms et devises</b>
              <p>Les invités et les capacités restent à leur table actuelle.</p>
            </div>
            <select aria-label="Première table à échanger" value={tableSwapFrom} onChange={(event) => setTableSwapFrom(Number(event.target.value))}>
              {tableNames.map((table, index) => (
                <option key={`from-${index}`} value={index + 1}>
                  Table {index} · {table.name}
                </option>
              ))}
            </select>
            <span>⇄</span>
            <select aria-label="Deuxième table à échanger" value={tableSwapTo} onChange={(event) => setTableSwapTo(Number(event.target.value))}>
              {tableNames.map((table, index) => (
                <option key={`to-${index}`} value={index + 1}>
                  Table {index} · {table.name}
                </option>
              ))}
            </select>
            <button className="primary" onClick={swapTableIdentities}>
              Échanger
            </button>
          </div>
          <div className="settingsgrid">
            {tableNames.map((t, i) => {
              ...
              return (
                <article key={tableNumber}>
                  <span>{String(displayTableNumber(tableNumber)).padStart(2, "0")}</span>
                  <label>
                    Nom de la table
                    <input value={t.name} onChange={(event) => updateTableDefinition(i, "name", event.target.value)} />
                  </label>
                  <label>
                    Devise
                    <input value={t.motto} onChange={(event) => updateTableDefinition(i, "motto", event.target.value)} />
                  </label>
                  <label>
                    Capacité
                    <input
                      ...
                      min={1}
                      max={20}
                      value={getTableCapacity(tableNumber)}
                      onChange={(e) => {
                        const next = Math.max(1, Math.min(20, Number(e.target.value) || 1));
                        setTableCapacities((values) => ({
                          ...values,
                          [tableNumber]: next,
                        }));
                      }}
                  ...
                  <button onClick={() => notify(`${t.name} est sauvegardée automatiquement`)}>Enregistré</button>
                </article>
              );
            })}
          </div>
      ...
      {isAuthenticated && tab === "Règles" && (
        <section className="page narrow">
          <div className="heading">
            <div>
              <p className="eyebrow">LE MOTEUR D'ÉQUILIBRE</p>
              <h1>Règles & priorités</h1>
              <p>Les obligations passent toujours avant les préférences. Chaque changement est sauvegardé localement.</p>
            </div>
          </div>
          <div className="rules accordion-list">
            <details className="rule-accordion" open>
              <summary>
                <span>＋ Ajouter une séparation</span>
                <b>Nouvelle exclusion</b>
              </summary>
              <div className="accordion-content">
                <div className="rule-builder">
                  <span className="rule-kind">À séparer</span>
                  <input list="rule-guest-a" value={ruleA} onChange={(e) => setRuleA(e.target.value)} placeholder="Premier invité…" />
                  <datalist id="rule-guest-a">
                    {guests.map((g) => (
                      <option key={g.id} value={g.name} />
                    ))}
                  </datalist>
                  <input list="rule-guest-b" value={ruleB} onChange={(e) => setRuleB(e.target.value)} placeholder="Deuxième invité…" />
                  <datalist id="rule-guest-b">
                    {guests.map((g) => (
                      <option key={g.id} value={g.name} />
                    ))}
                  </datalist>
            ...
            <details className="rule-accordion">
              <summary>
                <span>💍 Couples</span>
                <b>{coupleRules.length}</b>
              </summary>
              <div className="accordion-content">
                {coupleRules.length ? (
                  coupleRules.map((group) => (
                    <div
                      className="rule couple-rule"
                      key={group
                        .map((g) => g.id)
                        .sort()
                        .join("-")}
                    >
                      <b>Couple</b>
                      <p>{group.map((g) => g.name).join(" + ")}</p>
                      <span>Même table</span>
                      <button aria-label="Supprimer le couple" onClick={() => removeTogetherGroup(group)}>
                        ×
                      </button>
                    </div>
                  ))
                ) : (
                  <p className="empty-rules">Aucun couple enregistré.</p>
                )}
              </div>
            </details>
            <details className="rule-accordion">
              <summary>
                <span>↔ À séparer</span>
                <b>{separationRules.length}</b>
              </summary>
              <div className="accordion-content">
                {separationRules.length ? (
                  separationRules.map((rule) => {
                    const a = guests.find((g) => g.id === rule.aId);
                    const b = guests.find((g) => g.id === rule.bId);
                    return a && b ? (
                      <div className="rule separate-rule" key={rule.id}>
                        <b>Exclusion</b>
                        <p>
                          {a.name} ≠ {b.name}
                        </p>
                        <span>Tables distinctes</span>
                        <button aria-label="Supprimer l’exclusion" onClick={() => setSeparationRules((rules) => rules.filter((item) => item.id !== rule.id))}>
                          ×
                        </button>
                      </div>
                    ) : null;
                  ...
                  <p className="empty-rules">Aucune séparation enregistrée.</p>
                )}
              </div>
            </details>
            <details className="rule-accordion">
              <summary>
                <span>⚖ Critères d’équilibre</span>
                <b>{Object.keys(priorities).length}</b>
              </summary>
              <div className="accordion-content">
                {Object.entries(priorities).map(([name, value]) => (
                  <div className="slider" key={name}>
                    <label>
                      <b>{name}</b>
                      <span>Priorité {value > 70 ? "forte" : value > 40 ? "moyenne" : "faible"}</span>
                    </label>
                    <input
                      type="range"
                      min="0"
      ...
      {showLogin && !isAuthenticated && (
        <div
          className="modal-backdrop auth-modal"
          role="presentation"
          onMouseDown={(event) => {
            if (event.target === event.currentTarget) setShowLogin(false);
          }}
        >
          <section className="access-card" role="dialog" aria-modal="true" aria-labelledby="auth-title">
            <button className="auth-close" type="button" aria-label="Fermer" onClick={() => setShowLogin(false)}>
              ×
            </button>
            <div className="access-brand">
              ...
              <div>
                <strong>Place Parfaite</strong>
                <small>MODE ÉDITION</small>
              </div>
            </div>
            <p className="eyebrow">ACCÈS PRIVÉ</p>
            <h1 id="auth-title">Se connecter</h1>
            <p>Entrez le mot de passe partagé pour modifier le plan.</p>
            <form onSubmit={authenticate}>
              ...
              />
              {accessError && (
                <span id="access-error" role="alert">
                  {accessError}
                </span>
              )}
      ...
      {guestModalOpen && (
        <div
          ...
          role="presentation"
          onMouseDown={(event) => {
            if (event.target === event.currentTarget) setGuestModalOpen(false);
          }}
        >
          <section className="guest-modal" role="dialog" aria-modal="true" aria-labelledby="new-guest-title">
            <div className="modal-handle" aria-hidden="true" />
            <header>
              <div>
                <p className="eyebrow">{editingGuestId !== null ? "MODIFICATION" : "NOUVELLE FICHE"}</p>
                <h2 id="new-guest-title">{editingGuestId !== null ? "Modifier l’invité" : "Ajouter un invité"}</h2>
              </div>
              <button type="button" aria-label="Fermer" onClick={() => setGuestModalOpen(false)}>
                ×
              </button>
            </header>
            <form onSubmit={saveGuest}>
                ...
                <div className="choice-chips">
                  {languages.map((language) => (
                    <label key={language}>
                      <input
                        type="checkbox"
                        checked={newGuest.languages.includes(language)}
                        onChange={() =>
                          setNewGuest((value) => ({
                            ...value,
                            languages: value.languages.includes(language) ? value.languages.filter((item) => item !== language) : [...value.languages, language],
                          }))
                        }
                      />
                      <span>{language}</span>
              ...
              <footer>
                <button type="button" className="outline" onClick={() => setGuestModalOpen(false)}>
                  Annuler
                </button>
                <button type="submit" className="primary">
                  {editingGuestId !== null ? "Enregistrer" : "Ajouter l’invité"}
                </button>
              </footer>
            </form>
          </section>
